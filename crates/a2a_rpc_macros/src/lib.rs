...
/// WASM-compatible async execution helper
///
/// This function provides a way to run async code in WASM environments
/// using LocalPool which is single-threaded and WASM-compatible.
fn generate_wasm_async_helper() -> proc_macro2::TokenStream {
    quote! {
        /// WASM-compatible async execution helper
        ///
        /// This function provides a simplified async execution model that works
        /// in WASM/Spin environments using LocalPool for single-threaded execution.
        #[doc(hidden)]
        pub fn wasm_compat_run_async<F, T>(future: F) -> Result<T, String>
        where
            F: std::future::Future<Output = T>,
        {
            ...
            // Run the future to completion using LocalPool
            // This works in WASM because it's single-threaded and doesn't require complex threading
            Ok(pool.run_until(future))
        }

        /// Alternative async execution using current_thread executor
        ///
        /// This is another WASM-compatible approach for running async code.
        #[doc(hidden)]
        pub fn wasm_compat_run_async_current_thread<F, T>(future: F) -> Result<T, Box<dyn std::error::Error>>
        where
            F: std::future::Future<Output = T>,
        {
            // Using tokio's current_thread runtime which is WASM-compatible
            // This approach works in Spin SDK environments
...
pub fn generate_a2a_commands(_input: TokenStream) -> TokenStream {
    // Get the current crate name for namespacing
    let _crate_name = std::env::var("CARGO_PKG_NAME")
    ...
    // Generate the WASM async helper
    let wasm_async_helper = generate_wasm_async_helper();
