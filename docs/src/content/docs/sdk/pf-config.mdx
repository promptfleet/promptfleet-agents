...
<Aside type="note">In `agent_sdk`, config loading is opt-in via the `config-loader` feature flag. Enable it with `agent_sdk = { package = "pf_agent_sdk", version = "0.1", features = ["config-loader"] }` to use `AgentBuilder::from_config_path()`. See the [Configuration guide](/promptfleet-agents/guides/config/) for details.</Aside>
