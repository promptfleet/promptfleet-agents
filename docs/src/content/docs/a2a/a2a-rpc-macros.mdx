...
<Aside type="note">Experimental — this crate is not yet integrated into the SDK pipeline. The API is unstable and may change significantly.</Aside>

<Aside type="tip">Proc-macro crate — runs at compile time on the host. Generated code is WASM-compatible with zero runtime overhead.</Aside>
