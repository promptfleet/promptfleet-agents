...
|--------|---------|----------|
| `wasm32-wasi` | SpinKube serverless (4.8 ms cold start, ~0.6 MB/agent) | Production deployment on Kubernetes |
| Native (`x86_64` / `aarch64`) | Tokio + Axum | Local development, testing, sub-agent delegation, AG-UI streaming |

The SDK uses `cfg(target_arch = "wasm32")` gates internally so your agent code stays the same — only the entry point and async runtime change. On WASM, Spin SDK provides the HTTP listener and async executor. On native, Tokio and Axum take over.
...
<Aside type="caution">
Some features are **native-only**: `event-stream`, `agui-stream`, `sub-agents`, and `redis-storage` require Tokio and will not compile on `wasm32-wasi`. The presets that include them (`a2a-agent`, `dual-agent`, `pf-agent`) work on native; on WASM, use `a2a-server` + `llm-engine` directly.
</Aside>
