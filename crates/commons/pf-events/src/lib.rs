//! Shared event envelope primitives and CloudEvents helpers.
//!
//! This crate intentionally stays transport-focused: generic envelope types,
//! JSON schema helpers, and small ergonomic builders. Domain-specific event
//! mapping stays in higher-level crates.
...
/// CloudEvents structured-mode JSON envelope with support for extension
/// attributes via top-level flattening.
...
impl<T> CloudEventEnvelope<T> {
    /// Create a JSON payload envelope with standard defaults.
