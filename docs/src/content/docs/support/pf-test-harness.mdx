...
<Aside type="caution">Native only — this crate requires `tokio`, `axum`, and `reqwest` and does **not** compile to WASM. It is excluded from WASM builds via `#![cfg(not(target_arch = "wasm32"))]`.</Aside>
