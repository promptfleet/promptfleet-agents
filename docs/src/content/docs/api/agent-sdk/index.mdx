...
When using the `structured-io` feature, the runtime configurator can attach a typed output
contract directly:
...
fn wire_structured_runtime(
    client: impl agent_sdk::agent::llm_orchestrator::IntoLlmInvoker
        + agent_sdk::agent::llm_orchestrator::IntoLlmStreamInvoker
        + Clone,
) -> SdkResult<()> {
    let mut agent = AgentBuilder::new("my-agent")?.build()?;
    agent
        .configure_llm_runtime(client, "gpt-4o-mini", ToolRegistry::new(), None, None, None)?
        .with_structured_output::<AnalysisOutput>("analysis_output", "analysis_output")?;
    Ok(())
}
```

Structured CloudEvent output uses `urn:promptfleet:schema:<schema_name>` as the default
`dataschema` convention.
